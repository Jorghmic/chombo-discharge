...
workdir=${{USERWORK}}/{job_name}_${{SLURM_JOB_ID}}
mkdir -p "$workdir"
...
# Get the name of the output (sim)
output_names={output_names}

# Check if the checkpoint file exists from the previous job
checkpoint_file="{checkpoint_dir}/${output_names}.check{prev_max_steps:07d}.2d.hdf5"

if [ -e "$checkpoint_file" ]; then
    cp -f "$checkpoint_file" $workdir/
fi
...
def get_param(text, name):
    ...
    return match.group(1)  # Changed to return the string (not float)


# ==============================
# SLURM submission helpers
# ==============================
def write_slurm_file(input_file, prev_max_steps, output_names):
    tmp = tempfile.NamedTemporaryFile(
        mode="w", delete=False, suffix=".slurm"
    )
    tmp.write(SLURM_TEMPLATE.format(job_name=JOB_NAME, input_file=input_file, checkpoint_dir=CHECKPOINT_DIR, prev_max_steps=prev_max_steps, output_names=output_names))
    tmp.close()
    return tmp.name
...
def main(phases):
    ...
    for idx, phase in enumerate(phases):
        ...
        max_steps = int(get_param(text, PARAM_MAXSTEPS))  # Ensure it's an integer
        output_names = get_param(text, PARAM_OUTPUT)  # Get the output_names for the checkpoint file
        ...
        # Set the previous max_steps to be used for the checkpoint file
        prev_max_steps = int(restart)

        # Write the SLURM file and submit the job
        slurm_file = write_slurm_file(new_input_file, prev_max_steps, output_names)
