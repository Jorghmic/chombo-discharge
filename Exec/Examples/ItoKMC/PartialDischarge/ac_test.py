...
def build_input_for_phase(base_text, phase_deg, *, potential0, block_steps):
...
def main(phases):
    ...
    base_slurm = write_slurm_file(base_input_file, base_job_name, "")
    ...
    print(f"  submitted base job {base_job_id} (restart=0, max_steps={block_steps})\n")
    ...
    for phase in dep_phases:
        ...
        dep_slurm = write_slurm_file(dep_input_file, dep_job_name, base_job_name)
        ...
        print(
            ...
            f"(restart=AUTO from chk)"
        )
        print(f"  submitted dependent job {dep_job_id} (afterok:{base_job_id})\n")
