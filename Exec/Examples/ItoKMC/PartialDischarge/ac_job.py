...
# User configuration
# ==============================
INPUT_FILE = Path("test.inputs")

JOB_NAME = "TriplePoint11"
...
# ==============================
# Embedded SLURM script template
# ==============================
...
# ==============================
# Editable restart logic
# ==============================
def restart_value(phase, index):
    """
    Decide what Driver.restart should be for this phase.

    Parameters
    ----------
    phase : float
        Phase value (e.g. 30, 60, 90)
    index : int
        Job index (0, 1, 2, ...)

    RETURN VALUE MUST BE A STRING that goes into the inputs file.
...
def write_slurm_file():
    ...
    tmp.write(SLURM_TEMPLATE.format(job_name=JOB_NAME))
    tmp.close()
    return tmp.name
...
def main(phases):
    ...
    for idx, phase in enumerate(phases):
        ...
        output_name = f"sim{int(phase)}"
        restart = restart_value(phase, idx)

        print(
            f"Phase {phase:6.1f}: "
            f"{potential:.3e} × sin({phase}°) → {new_potential:.3e}"
        )
        print(f"  Driver.output_names = {output_name}")
        print(f"  Driver.restart      = {restart}")

        text = set_param(text, PARAM_POTENTIAL, f"{new_potential:.8e}")
        text = set_param(text, PARAM_OUTPUT, output_name)
        text = set_param(text, PARAM_RESTART, restart)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python submit_chain.py 30 60 90")
        sys.exit(1)
